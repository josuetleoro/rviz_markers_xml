...
visualization_msgs::Marker MarkersXMLParser::parseCubeList(XMLElement* shape_element)
{
  ...
  for (vector<geometry_msgs::Point>::iterator it = cube_list.points.begin();
       it != cube_list.points.end(); it++)
  {
...
geometry_msgs::Pose MarkersXMLParser::parseElementPose(XMLElement* shape_element)
{
  ...
  {
    string error_msg =
      "Wrong number of elements [" + to_string(token_number) + "] for xyz attribute, expected 3.";
    throwErrorWithLine(error_msg, pose_element->GetLineNum());
  }
  ...
  {
    string error_msg =
      "Wrong number of elements [" + to_string(token_number) + "] for rpy attribute, expected 3.";
    throwErrorWithLine(error_msg, pose_element->GetLineNum());
  }

  ROS_INFO("Pose: %.3f %.3f %.3f %.3f %.3f %.3f", vec.at(0), vec.at(1), vec.at(2), vec.at(3),
           vec.at(4), vec.at(5));
  ...
  pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(
    vec.at(3) * deg2rad, vec.at(4) * deg2rad, vec.at(5) * deg2rad);
  return pose;
}
...
std_msgs::ColorRGBA MarkersXMLParser::parseElementColor(XMLElement* shape_element)
{
  ...
  while (ss)
  {
    ...
    {
      string error_msg =
        "Invalid value for rgba attribute: Value out of range. Valid range [0.0, 1.0]";
      throwErrorWithLine(error_msg, color_element->GetLineNum());
    }
  }
  if (token_number < 4)
  {
    string error_msg =
      "Wrong number of elements [" + to_string(token_number) + "] in rgba attribute, expected 4.";
    throwErrorWithLine(error_msg, color_element->GetLineNum());
  }
...
vector<geometry_msgs::Point> MarkersXMLParser::parseElementDash(XMLElement* shape_element)
{
  ...
  {
    string error_msg =
      "Invalid gap_ratio value in dash element: Value out of range. Valid range (0.0, 1.0)";
    throwErrorWithLine(error_msg, dash_element->GetLineNum());
  }
  ...
  {
    string error_msg =
      "Lenght of dashed_line smaller than 0.0001. Use points with greater distance between them.";
    throwErrorWithLine(error_msg, dash_element->GetLineNum());
  }
  dir.normalize();
